...
var Helper = function (_BaseGenerator) {
    _inherits(Helper, _BaseGenerator);
    ...
    function Helper() {
        ...
        _this.appRoot = _this.appRoot;
        return _this;
    }

    _createClass(Helper, [{
        key: 'appRoot',
        value: async function appRoot() {
            var root = void 0;
            var pathUrl = path.join(process.cwd(), 'ace');
            var exists = await fs.pathExists(pathUrl);
            if (exists) {
                root = process.cwd();
                return root;
            }
